...
function countryclicked() {
  ...
  const popupdiv = d3.select("#popup").transition(t)
    .style("opacity", "1")
    .style("display", "block")
  ...
  d3.select(".popup_text")
    .append("h1")
    .attr("class", ".popup-header")
      .text(state.hover_country)
    
  d3.select(".popup_text").append("div")
  ...
  countryLink = d3.select(".popup-body")
...
function cleanword(d) {
  cleaned = (((d.split(" ").join("_")).split(",").join("_")).split("(").join("_")).split(")").join("_")
  return cleaned
}

function popup_close() {
  d3.selectAll(".popup_text").remove();
...
function poplistonClick(ev,data) {
  d3.selectAll(".popup_text").remove();

  // console.log(data)

  filteredOrigins = filteredPopData.filter(d => (d.Country === data) || (data === d.Region))

  filteredOrigins = d3.rollup(filteredOrigins, v => v.length, d=>d.country)
  ...
  d3.select(".popup_text")
    ...
    const divlist = d3.select(".popup_text")
      .append("div")
      .attr("class", "popup-body")
      .style("width", "100%")
    ...
    // })
      ...
      //   .join("p")
      //   .attr("class", "countryName")
      //   .attr("id", d=>((d.split(" ").join("_")).split(",").join("")))
      //   .text(function(d){
      //     if (groupedCountry.get(d) === undefined) {
      //       floodnum = groupedRegion.get(d)
      //     }
