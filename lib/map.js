...
let width = (+d3.select('#map-container').style('width').slice(0, -2)),
  ...
  margin= 0,
  projection, pathGen, mapsvg, countries, colorScale, colorScaleDark, tooltip, scale=.97, regionNameGroup = {}, filteredPopData, groupedArea, groupedCountry, groupedRegion, currHoverpopList, countryLink, floodpins, pageName, currPage, regions, floodnum, prevPage, cleaned, filteredOrigins, filteredOriginsRoll, filteredStories, story;

var mapdefs, mapfilter, mapSVGsize, gSize, x, y, mapfeMerge, flooddef;
...
function draw() {
  ...
  mapdefs = mapsvg.append("defs");

  mapfilter = mapdefs.append("filter")
      .attr("id", "dropshadow")
  ...
  mapfeMerge = mapfilter.append("feMerge");

  mapfeMerge.append("feMergeNode")
      .attr("in", "offsetBlur")
  mapfeMerge.append("feMergeNode")
  ...
  // centering map
  mapSVGsize = d3.select(".mapsvg").node().getBoundingClientRect()
  gSize = d3.select("g").node().getBoundingClientRect()

  x = (mapSVGsize.width/2)-((gSize.width*scale)/2)+(-gSize.x);
  y = 0;
